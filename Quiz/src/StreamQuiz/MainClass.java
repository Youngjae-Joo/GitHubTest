...
//		}
		...
		//1.트래픽의 합 
		try {
			BufferedReader bfr = new BufferedReader(new FileReader("C:\\Users\\user\\Desktop\\log.txt"));
			...
			while(true) {
				...
				if(line==null) break;
				String traffic =line.split(" ")[line.split(" ").length-1];
				traffic=traffic.replaceAll("[^0-9]", "0");
				if(Integer.parseInt(traffic)!=0) {
					map.put(line.split(" ")[0], Integer.parseInt(traffic));
					sum+=Integer.parseInt(traffic);
				}
			}
			bfr.close();
